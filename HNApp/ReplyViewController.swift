...
class ReplyViewController: UIViewController {
    ...
    @IBAction func sendButtonDidTouch(sender: AnyObject) {
        ...
        if let storyId = story["id"].int {
            HNService.replyStoryWithId(storyId, token: token, body: body, response: { (successful) -> () in
        ...
        if let commentId = comment["id"].int {
            HNService.replyCommentWithId(commentId, token: token, body: body, response: { (successful) -> () in
                self.view.hideLoading()
                if successful {
                    self.dismissViewControllerAnimated(true, completion: nil)
                    self.delegate?.replyViewControllerDidSend(self)
